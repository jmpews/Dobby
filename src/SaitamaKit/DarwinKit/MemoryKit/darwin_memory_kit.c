...
bool zz_darwin_vm_read_data_via_task(task_t task, zz_addr_t address, zz_ptr_t buffer, zz_size_t length) {
    vm_size_t n;
...
char *zz_darwin_vm_read_string_via_task(task_t task, zz_addr_t address) {
    char end_c = '\0';
    unsigned long end_addr;
    char *result = NULL;

    // string upper limit 0x1000
    end_addr = zz_darwin_vm_search_data_via_task(task, address, address + 0x1000, &end_c, 1);
    if (!end_addr) {
        return NULL;
    }
    result = (char *)malloc(end_addr - address + 1);
    if (result && zz_darwin_vm_read_data_via_task(task, address, result, end_addr - address + 1)) {
        return result;
    }
    return NULL;
}

zz_addr_t zz_darwin_vm_search_data_via_task(task_t task, zz_addr_t start_addr, zz_addr_t end_addr, char *data,
                                            zz_size_t data_len) {
    ...
    while (end_addr > curr_addr) {
        if (zz_darwin_vm_read_data_via_task(task, curr_addr, temp_buf, data_len))
...
bool zz_darwin_vm_check_address_valid_via_task(task_t task, const zz_addr_t address) {
    if (address <= 0)
        return FALSE;
...
bool zz_darwin_vm_get_page_info_via_task(task_t task, const zz_addr_t address, vm_prot_t *prot_p,
...
bool zz_darwin_vm_protect_via_task(task_t task, const zz_addr_t address, zz_size_t size, vm_prot_t page_prot) {
    kern_return_t kr;
    vm_size_t page_size;
...
bool zz_darwin_vm_protect_as_executable_via_task(task_t task, const zz_addr_t address, zz_size_t size) {
    return zz_darwin_vm_protect_via_task(task, address, size, (VM_PROT_READ | VM_PROT_EXECUTE));
}

bool zz_darwin_vm_protect_as_writable_via_task(task_t task, const zz_addr_t address, zz_size_t size) {
    if (!zz_darwin_vm_protect_via_task(task, address, size, (VM_PROT_ALL | VM_PROT_COPY))) {
        return zz_darwin_vm_protect_via_task(task, address, size, (VM_PROT_DEFAULT | VM_PROT_COPY));
    }
    return FALSE;
}

zz_ptr_t zz_darwin_vm_allocate_pages_via_task(task_t task, zz_size_t n_pages) {
    vm_address_t result;
    kern_return_t kr;
    vm_size_t page_size;
    page_size = zz_posix_vm_get_page_size();
    ...
    if (!zz_darwin_vm_protect_via_task(task, result, page_size * n_pages, (VM_PROT_DEFAULT | VM_PROT_COPY)))
        return NULL;
...
bool zz_darwin_vm_can_allocate_rx_page() {
    vm_prot_t prot;
    ...
    vm_address_t temp_page_addr = (vm_address_t)zz_darwin_vm_allocate_pages_via_task(mach_task_self(), 1);
    zz_darwin_vm_protect_as_executable_via_task(mach_task_self(), temp_page_addr, zz_posix_vm_get_page_size());

    if (!zz_darwin_vm_get_page_info_via_task(task_self, temp_page_addr, &prot, &inherit)) {
        return FALSE;
    }
...
zz_ptr_t zz_darwin_vm_allocate_via_task(task_t task, zz_size_t size) {
    ...
    result = zz_darwin_vm_allocate_pages_via_task(task, n_pages);
    return result;
}

zz_ptr_t zz_darwin_vm_allocate_near_pages_via_task(task_t task, zz_addr_t address, zz_size_t range_size,
                                                   zz_size_t n_pages) {
...
zz_ptr_t zz_darwin_vm_search_text_code_cave_via_task(task_t task, zz_addr_t address, zz_size_t range_size,
                                                     zz_size_t *size_ptr) {
    char zeroArray[128];
...
ZMemoryLayout *zz_darwin_vm_get_memory_layout_via_task(task_t task) {
    ...
    ZMemoryLayout *mlayout = (ZMemoryLayout *)malloc(sizeof(ZMemoryLayout));
    memset(mlayout, 0, sizeof(ZMemoryLayout));
...
zz_ptr_t zz_darwin_vm_search_code_cave(zz_addr_t address, zz_size_t range_size, zz_size_t size) {
    char zeroArray[128];
    char readZeroArray[128];
    ...
    ZMemoryLayout *mlayout = zz_darwin_vm_get_memory_layout_via_task(mach_task_self());
...
zz_ptr_t zz_darwin_vm_search_text_code_cave_via_dylibs(zz_addr_t address, zz_size_t range_size, zz_size_t size) {
    char zeroArray[128];
    char readZeroArray[128];
    vm_address_t aligned_addr, tmp_addr, search_start, search_end, search_start_limit, search_end_limit;
    vm_size_t page_size;
...
bool zz_darwin_vm_patch_code_via_task(task_t task, const zz_addr_t address, const zz_ptr_t codedata,
                                      zz_size_t codedata_size) {
    ...
    if (!zz_darwin_vm_get_page_info_via_task(task_self, (const zz_addr_t)start_page_addr, &prot, &inherit)) {
        return FALSE;
    }
    ...
    void *code_mmap = zz_darwin_vm_allocate_via_task(task_self, range_size);
    ...
    if (!zz_darwin_vm_protect_via_task(task_self, (zz_addr_t)code_mmap, range_size, prot))
        return FALSE;
