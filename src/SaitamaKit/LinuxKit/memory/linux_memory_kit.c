...
ZMemoryLayout *zz_linux_vm_get_memory_layout_via_pid(pid_t pid) {
    char filename[64];
    char buf[256];
    FILE *fp;
    ZMemoryLayout *mlayout;

    mlayout = (ZMemoryLayout *)malloc(sizeof(ZMemoryLayout));
...
zz_ptr_t zz_linux_vm_search_code_cave(zz_addr_t address, zz_size_t range_size, zz_size_t size) {
    ...
    ZMemoryLayout *mlayout = zz_linux_vm_get_memory_layout_via_pid(-1);
