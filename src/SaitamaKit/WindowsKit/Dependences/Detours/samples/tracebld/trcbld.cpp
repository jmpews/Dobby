...
struct FileInfo
{
    ...
    BOOL    m_fCanThread;        // Set for file that are opened Create
    BOOL    m_fRead;
    BOOL    m_fWrite;
...
class OpenFiles
{
  ...
  public:
    ...
    static VOID SeThread(HANDLE hFile, DWORD cbData)
    {
        SLOT& slot = HashToSlot(hFile);
        if (slot.m_hHandle == hFile) {
