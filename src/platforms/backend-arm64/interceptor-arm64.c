...
void TrampolinePrepare(InterceptorBackend *self, HookEntry *entry) {
    ...
    ZzARM64HookEntryBackend *entry_backend;

    entry_backend  = (ZzARM64HookEntryBackend *)malloc0(sizeof(ZzARM64HookEntryBackend));
...
void TrampolineBuildForEnterTransfer(InterceptorBackend *self, HookEntry *entry) {
    ...
    ZzARM64HookEntryBackend *entry_backend = (ZzARM64HookEntryBackend *)entry->backend;
    ZZSTATUS status                        = ZZ_SUCCESS;
    zz_addr_t target_addr                  = (zz_addr_t)entry->target_ptr;

    arm64_writer = &self->arm64_writer;
    arm64_writer_reset(arm64_writer, temp_code_slice, 0);
    ...
    if (entry_backend->redirect_code_size == ZZ_ARM64_TINY_REDIRECT_SIZE) {
        code_slice = arm64_code_patch(arm64_writer, self->emm, target_addr, arm64_writer_near_jump_range_size() - 0x10);
    } else {
        code_slice = arm64_code_patch(arm64_writer, self->emm, 0, 0);
    }
...
void TrampolineBuildForEnter(InterceptorBackend *self, HookEntry *entry) {
    ...
    arm64_writer_put_sub_reg_reg_imm(arm64_writer, ZZ_ARM64_REG_SP, ZZ_ARM64_REG_SP, 2 * 0x8);
    arm64_writer_put_ldr_b_reg_address(arm64_writer, ZZ_ARM64_REG_X17, (zz_addr_t)entry);
    arm64_writer_put_str_reg_reg_offset(arm64_writer, ZZ_ARM64_REG_X17, ZZ_ARM64_REG_SP, 0x0);

    // jump to enter thunk
    arm64_writer_put_ldr_br_reg_address(arm64_writer, ZZ_ARM64_REG_X17, (zz_addr_t)self->enter_thunk);
...
void TrampolineBuildForInvoke(InterceptorBackend *self, HookEntry *entry) {
    ...
    if (entry->hook_type == HOOK_TYPE_ONE_INSTRUCTION) {
        ...
        arm64_writer_put_ldr_br_reg_address(arm64_writer, ZZ_ARM64_REG_X17, (zz_addr_t)entry->on_insn_leave_trampoline);
    ...
    code_slice = arm64_relocate_code_patch(arm64_relocator, arm64_writer, self->emm, 0, 0);
    if (code_slice)
...
void TrampolineBuildForInstructionLeave(InterceptorBackend *self, HookEntry *entry) {
    ...
    arm64_writer_put_ldr_br_reg_address(arm64_writer, ZZ_ARM64_REG_X17, (zz_addr_t)self->insn_leave_thunk);
...
void TrampolineBuildForLeave(InterceptorBackend *self, HookEntry *entry) {
    ...
    arm64_writer_put_ldr_br_reg_address(arm64_writer, ZZ_ARM64_REG_X17, (zz_addr_t)self->leave_thunk);
...
void TrampolineActivateAll(InterceptorBackend *self, HookEntry *entry) {
    ...
    arm64_writer = &self->arm64_writer;
    arm64_writer_reset(arm64_writer, temp_code_slice, target_addr);
    ...
    if (!MemoryPatchCode((zz_addr_t)target_addr, (zz_ptr_t)arm64_writer->w_start_address, arm64_writer->size))
        status = ZZ_FAILED;
...
ZZSTATUS ZzActivateStaticBinaryInstrumentationTrampoline(HookEntry *entry, zz_addr_t target_fileoff) {
