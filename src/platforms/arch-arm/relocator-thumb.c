...
void thumb_relocator_init(ThumbRelocator *relocator, ARMReader *input, ThumbAssemblyWriter *output) {
  ...
  if (relocator->literal_insnCTXs_count) {
    ...
    for (int i = 0; i < relocator->literal_insnCTXs_count; i++) {
      ...
      if (*literal_target_address_ptr > (relocator->input->start_pc - 4) &&
        ...
        relocated_insn = thumb_relocator_get_relocator_insn_with_address(relocator, *literal_target_address_ptr);
...
void thumb_relocator_write_all(ThumbRelocator *self) {
  ...
  ThumbAssemblyWriter thumb_writer = *self->output;
...
bool thumb_relocator_double_write(ThumbRelocator *self, zz_addr_t final_address) {
  ...
  ThumbAssemblyWriter *writer = self->output;
