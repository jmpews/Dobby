...
ZZSTATUS ZzBuildEnterTrampoline(ZzInterceptorBackend *self,
    ...
    zz_thumb_writer_put_ldr_reg_reg_offset(thumb_writer, ARM_REG_R7, ARM_REG_SP,
                                           0 * 4);
    ...
    if (is_thumb &&
        ...
        code_slice =
            ZzNewNearCodeSlice(self->allocator, (zaddr)entry->target_ptr,
                               ...
                               thumb_writer->size); // @common-function
        if (!code_slice)
            return ZZ_FAILED;
    }
    ...
    code_slice =
        ZzNewCodeSlice(self->allocator, thumb_writer->size); // @common-funciton
    if (!code_slice)
        return ZZ_FAILED;
...
ZZSTATUS ZzBuildInvokeTrampoline(ZzInterceptorBackend *self,
    ...
    if (is_thumb) {
        ...
        do {
            ...
            tmp_relocator_insn_size =
                arm_relocator->input_cur - arm_relocator->input_start;
        } while (tmp_relocator_insn_size < entry_backend->redirect_code_size);
...
ZZSTATUS ZzBuildLeaveTrampoline(ZzInterceptorBackend *self,
    ...
    code_slice =
        ZzNewCodeSlice(self->allocator, thumb_writer->size); // @common-function
    ...
    if (is_thumb) {
        ...
        if (entry_backend->redirect_code_size == ZZ_THUMB_TINY_REDIRECT_SIZE) {
            ...
            zz_thumb_writer_put_ldr_reg_address(
                thumb_writer, ARM_REG_PC, (zaddr)entry->on_enter_trampoline);
        }
    } else {
        ZzArmWriter *arm_writer;
        arm_writer = &self->arm_writer;
        ...
        if (entry_backend->redirect_code_size == ZZ_ARM_TINY_REDIRECT_SIZE) {
            ...
            zz_arm_writer_put_ldr_reg_address(
                arm_writer, ARM_REG_PC, (zaddr)entry->on_enter_trampoline);
        }
    }
