...
static ZzARM64RelocatorInstruction *zz_arm64_relocator_get_relocator_insn_with_address(ZzARM64Relocator *self,
        ...
        for (int i = 0; i < relocator->literal_insn_size; i++) {
            ...
            if (*literal_target_address_ptr > relocator->input->start_pc &&
                *literal_target_address_ptr < (relocator->input->start_pc + relocator->input->size)) {
                relocated_insn =
                    zz_arm64_relocator_get_relocator_insn_with_address(relocator, *literal_target_address_ptr);
