...
template <typename T> class RingBuffer {
  class Iterator {
    ...
    Iterator(int N, T *ptr, int pos) : N(N), ptr(ptr), pos(pos) {
    }
  ...
  public:
    ...
    Iterator operator+(const int n) {
      Iterator it(N, this->ptr, this->pos + n);
      return it;
    }
    Iterator operator-(const Iterator &rhs) {
      Iterator it(N, this->ptr, this->pos - rhs.pos);
      return it;
    }
    Iterator operator-(const int n) {
      Iterator it(N, this->ptr, this->pos - n);
    ...
    bool operator==(const Iterator &rhs) const {
      return (rhs.ptr == ptr) && (rhs.pos % N == pos % N);
    }
    bool operator!=(const Iterator &rhs) const {
      return !(*this == rhs);
    }
...
public:
  ...
  RingBuffer() : N(16), queue_(new T[N]), head_(N, queue_, 0), tail_(N, queue_, 0) {
  }

  RingBuffer(std::initializer_list<T> lst) : N(16), queue_(new T[N]), head_(N, queue_, 0), tail_(N, queue_, 0) {
  ...
  // copy
  explicit RingBuffer(const RingBuffer &r) : N(r.N), queue_(new T[N]), head_(r.head_), tail_(r.tail_) {
    head_.ptr = queue_;
    tail_.ptr = queue_;
  ...
  size_t size() const {
    return (size_t)abs(tail_.raw_pos() - head_.raw_pos());
  }
  ...
  iterator begin() {
    return empty() ? Iterator(N) : head_;
  }
  iterator end() {
    return empty() ? Iterator(N) : tail_;
  }
...
template <typename T> bool operator==(const RingBuffer<T> &x, const RingBuffer<T> &y) {
  if (x.size() != y.size())
    return false;
...
template <typename T> struct vector : public RingBuffer<T> {
  using iterator = typename RingBuffer<T>::iterator;
  using const_iterator = typename RingBuffer<T>::const_iterator;

  vector() : RingBuffer<T>() {
  }
  vector(std::initializer_list<T> lst) : RingBuffer<T>(lst) {
  }

  // copy
  vector(const vector &r) : RingBuffer<T>(r) {
  }

  vector &operator=(const vector &r) {
    RingBuffer<T>::operator=(r);
    return *this;
  }

  // move
  vector(vector &&r) : RingBuffer<T>(r) {
  }
...
private:
  using RingBuffer<T>::pop;
  using RingBuffer<T>::pop_front;
  using RingBuffer<T>::push;
  using RingBuffer<T>::push_front;
  using RingBuffer<T>::emplace;
};

template <typename T> struct deque : public RingBuffer<T> {
  using iterator = typename RingBuffer<T>::iterator;
  using const_iterator = typename RingBuffer<T>::const_iterator;

  deque() : RingBuffer<T>() {
  }
  deque(std::initializer_list<T> lst) : RingBuffer<T>(lst) {
  }

  // copy
  deque(const deque &r) : RingBuffer<T>(r) {
  }
...
private:
  using RingBuffer<T>::capacity;
  using RingBuffer<T>::pop;
  using RingBuffer<T>::push;
  using RingBuffer<T>::emplace;
  using RingBuffer<T>::assign;
  using RingBuffer<T>::begin;
  using RingBuffer<T>::end;
};
...
template <class T1, class T2> pair<T1, T2> make_pair(const T1 &t1, const T2 &t2) {
  return {t1, t2};
};
...
template <class Key, class T> struct map : public RingBuffer<pair<Key, T>> {
  using iterator = typename RingBuffer<pair<Key, T>>::iterator;
  using const_iterator = typename RingBuffer<pair<Key, T>>::const_iterator;
  using base = RingBuffer<pair<Key, T>>;
...
template <typename T> using ArxRingBuffer = arx::RingBuffer<T>;
