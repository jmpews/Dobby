...
void FunctionWrapperRouting::BuildPreCallRouting() {
  ...
  HOOKZZ_DLOG("[*] create pre call closure trampoline to 'prologue_routing_dispatch' at %p\n", cte->address);
}
...
void FunctionWrapperRouting::BuildPostCallRouting() {
  ...
  HOOKZZ_DLOG("[*] create post call closure trampoline to 'prologue_routing_dispatch' at %p\n",
