...
void ZzSolidifyHookInitilize(MachoFD *machofd) {
...
void ZzSolidifyBuildEnterTrampoline(ZzInterceptorBackendNoJB *nojb_backend, ZzHookFunctionEntryNoJB *nojb_entry,
    ...
    zaddr entry_address_offset;
    zaddr enter_thunk_address_offset;
    ...
    zaddr entry_address = (zaddr)data_seg_info->vmaddr + sizeof(ZzInterceptorBackendNoJB) +
                          nojb_backend->num_of_entry * sizeof(ZzHookFunctionEntryNoJB);
    zaddr codepatch_entry_address = code_seg_info->vmaddr + nojb_backend->code_seg_offset + 5 * 4;
    entry_address_offset = entry_address - codepatch_entry_address;
    *(void **)&on_enter_trampoline_tp[5 * 4] = (void *)entry_address_offset;

    zaddr enter_thunk_address = data_seg_info->vmaddr + 0x8 * 0;
    zaddr codepatch_enter_thunk_address = code_seg_info->vmaddr + nojb_backend->code_seg_offset + 15 * 4;
    enter_thunk_address_offset = enter_thunk_address - codepatch_enter_thunk_address;
    *(void **)&on_enter_trampoline_tp[15 * 4] = (void *)enter_thunk_address_offset;

    zz_file_write(target_path, nojb_backend->code_seg_offset, on_enter_trampoline_tp, ENTER_TRAMPOLINE_SIZE);
    nojb_entry->on_enter_trampoline = (void *)nojb_backend->code_seg_offset;
...
void ZzSolidifyHook(zpointer target_addr, MachoFD *machofd) {
    const char *target_path = machofd->getPath();
    const segment_command_64_info_t *code_seg_info = machofd->get_seg_by_name((char *)"HookZzCode");
    const segment_command_64_info_t *data_seg_info = machofd->get_seg_by_name((char *)"HookZzData");
    ...
    Xinfo("[*] start solidify hook at %p", target_addr);
    nojb_entry.target_address = target_addr;
...
int main(int args, const char **argv) {
    ...
    machofd->parse_macho();
    ZzSolidifyHookInitilize(machofd);
